...
local DEFAULT_TRACK_VOLUME = 0.14
local TRACK_VOLUMES = {
  ["AI Drummer"] = 0.30,
  ["AI Bass Player"] = 0.20,
...
local function add_or_find_fx(track, name, needle)
  local fx_index = find_fx(track, needle)
...
local function set_reasynth(track, fx_index, track_name_value)
  local volume = 0.04
  local attack = 0.10
  local decay = 0.24
  local sustain = 0.18
...
local function set_drum_synth(track, fx_index, track_name_value)
...
for index = 0, reaper.CountTracks(0) - 1 do
  ...
  if not should_skip(name) then
    ...
    if name == "AI Drummer" or name == "AI Percussion Extras" then
      local fx_index, was_added = add_or_find_fx(track, "JS: AI Band GM Drum Synth", "AI Band GM Drum Synth")
      if fx_index >= 0 then
        set_drum_synth(track, fx_index, name)
        configured = configured + 1
        if was_added then added = added + 1 end
      end
    else
      local fx_index, was_added = add_or_find_fx(track, "ReaSynth", "ReaSynth")
      if fx_index >= 0 then
        set_reasynth(track, fx_index, name)
        configured = configured + 1
        if was_added then added = added + 1 end
      end
    end
